...
// Funcție pentru găsirea indexului unui comentariu în array-ul de comentarii
function findCommentIndex(comments, name, comment) {
  for (var i = 0; i < comments.length; i++) {
    if (comments[i].name === name && comments[i].comment === comment) {
      return i;
    }
  }
  return -1;
...
commentForm.addEventListener("submit", function(event) {
  if (!commentForm.checkValidity()) {
    event.preventDefault(); // Prevenirea trimiterii formularului dacă nu este valid
  }
});
